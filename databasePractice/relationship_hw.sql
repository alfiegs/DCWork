...
-- SELECT tech.name, count(project_uses_tech.project_id) FROM tech LEFT OUTER JOIN project_uses_tech ON project_uses_tech.tech_id = tech.id GROUP BY tech.id ORDER BY count desc;

-- --5) Perform a left outer join from the project table to the project_users_tech table - which projects has no associated tech?
-- SELECT project.name, project_uses_tech.tech_id FROM project LEFT OUTER JOIN project_uses_tech ON project_uses_tech.project_id = project.id;
-- --Whiteboard Exercises

--6) Based on the previous query, get the count of the number of projects that use each tech.
-- SELECT project.name, count(project_uses_tech.tech_id) FROM project LEFT OUTER JOIN project_uses_tech ON project_uses_tech.project_id = project.id GROUP BY project.id ORDER BY count desc;

--7) List all projects along with each technology used by it. You will need to do a three-way join.
-- SELECT project.name, tech.name FROM project JOIN project_uses_tech ON project.id = project_uses_tech.project_id JOIN tech ON tech.id = project_uses_tech.tech_id;

--8) List all the distinct techs that are used by at least one project.
-- SELECT tech.name FROM project JOIN project_uses_tech ON project.id = project_uses_tech.project_id JOIN tech ON tech.id = project_uses_tech.tech_id GROUP BY tech.name;
...
-- SELECT project.name FROM project JOIN project_uses_tech ON project.id = project_uses_tech.project_id JOIN tech ON tech.id = project_uses_tech.tech_id GROUP BY project.name;
...
-- SELECT project.name, COUNT(project_uses_tech.tech_id) FROM project JOIN project_uses_tech ON project.id = project_uses_tech.project_id JOIN tech ON tech.id = project_uses_tech.tech_id GROUP BY project.name ORDER BY count DESC;

--13) Order the tech by how many projects use it.
-- SELECT tech.name, COUNT(project_uses_tech.project_id) FROM project JOIN project_uses_tech ON project.id = project_uses_tech.project_id JOIN tech ON tech.id = project_uses_tech.tech_id GROUP BY tech.name ORDER BY count DESC;

--???????????????????????????????????????????????????????????????
--14) What is the average number of techs used by a project?
-- SELECT AVG(tech.id) FROM project JOIN project_uses_tech ON project.id = project_uses_tech.project_id JOIN tech ON tech.id = project_uses_tech.tech_id;
